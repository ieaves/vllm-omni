...
# https://github.com/huggingface/diffusers/blob/main/src/diffusers/pipelines/cosmos/pipeline_cosmos2_5_transfer.py
...
from vllm_omni.diffusion.models.cosmos.utils import DEFAULT_NEGATIVE_PROMPT, CosmosSafetyChecker, retrieve_latents, xm

logger = logging.getLogger(__name__)


def _maybe_pad_or_trim_video(video: torch.Tensor, num_frames: int) -> torch.Tensor:
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> prompt = (
        ...     "The video is a demonstration of robotic manipulation, likely in a laboratory or testing environment. "
        ...     "It features two robotic arms interacting with a piece of blue fabric. The setting is a room with a "
        ...     "beige couch in the background, providing a neutral backdrop for the robotic activity. The robotic "
        ...     "arms are positioned on either side of the fabric, which is placed on a yellow cushion. The left "
        ...     "robotic arm is white with a black gripper, while the right arm is black with a more complex, "
        ...     "articulated gripper. At the beginning, the fabric is laid out on the cushion. The left robotic arm "
        ...     "approaches the fabric, its gripper opening and closing as it positions itself. The right arm remains "
        ...     "stationary initially, poised to assist. As the video progresses, the left arm grips the fabric, "
        ...     "lifting it slightly off the cushion. The right armthen moves in, its gripper adjusting to grasp the "
        ...     "opposite side of the fabric. Both arms work in coordination, lifting and holding the fabric between "
        ...     "them. The fabric is manipulated with precision, showcasing the dexterity and control of the robotic "
        ...     "arms. The camera remains static throughout, focusing on the interaction between the robotic arms and "
        ...     "the fabric, allowing viewers to observe the detailed movements and coordination involved in the task."
        ... )
        >>> negative_prompt = (
...
class Cosmos2_5_TransferPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        vae_scale_factor_temporal: int = 2 ** sum(self.vae.temperal_downsample) if getattr(self, "vae", None) else 4
        vae_scale_factor_spatial: int = 2 ** len(self.vae.temperal_downsample) if getattr(self, "vae", None) else 8
    ...
    def _get_prompt_embeds(
        ...
        prompt = [prompt] if isinstance(prompt, str) else prompt or []
    ...
    # Modified from diffusers.pipelines.cosmos.pipeline_cosmos2_video2world.Cosmos2VideoToWorldPipeline.prepare_latents
    # and diffusers.pipelines.cosmos.pipeline_cosmos2_video2world.Cosmos2TextToImagePipeline.prepare_latents
    def prepare_latents(
        self,
    ...
    def check_inputs(
        ...
        if callback_on_step_end_tensor_inputs is not None and not all(
            ...
            raise ValueError(
                "`callback_on_step_end_tensor_inputs` has to be in {self._callback_tensor_inputs}, "
        ...
        min_chunk_len: int = self.vae_scale_factor_temporal + 1
        if not num_frames_per_chunk or num_frames_per_chunk < min_chunk_len:
        ...
        if num_ar_conditional_frames is None or num_ar_conditional_frames >= num_frames_per_chunk:
            raise ValueError(
    ...
    def __call__(
        ...
        num_videos_per_prompt: int = 1,
        generator: torch.Generator | list[torch.Generator] | None = None,
        latents: torch.Tensor | None = None,  # type: ignore
        prompt_embeds: torch.Tensor | None = None,
        negative_prompt_embeds: torch.Tensor | None = None,
        ...
        if output_type is None:
            output_type = "pil"
        output_type = cast(str, output_type)
        ...
        if num_ar_latent_conditional_frames is not None:
            ...
            num_ar_conditional_frames = cast(int, num_ar_conditional_frames)
        ...
        if control_video.shape[0] != batch_size:
            if control_video.shape[0] == 1:
                ...
                size = control_video.shape[0]
                raise ValueError(
                    f"Expected controls batch size {batch_size} to match prompt batch size, but got {size}."
                )
        ...
        with self.progress_bar(total=total_steps) as progress_bar:
            for chunk_idx, (start_idx, end_idx) in enumerate(chunk_idxs):
                ...
                for i, t in enumerate(timesteps):
                    ...
                    latents: torch.Tensor = self.scheduler.step(noise_pred, t, latents, return_dict=False)[0]
        ...
        if output_type != "latent":
            ...
            self.safety_checker.to(device)
            video = cast(np.ndarray, self.video_processor.postprocess_video(video, output_type="np"))
        ...
        return CosmosPipelineOutput(frames=video)  # type: ignore
